...
import { XMarkIcon, MagnifyingGlassIcon, ExclamationTriangleIcon, CheckCircleIcon, LinkIcon } from '@heroicons/react/24/solid';
...
const ArticleCard = ({ article, index, darkMode = false }) => {
  ...
  return (
    <div 
      className="relative w-full h-96 card-enter-animation"
    ...
    >
      ...
      >
        ...
        <div className={`w-full h-full border-2 border-slate-600 ${darkMode ? 'bg-slate-800 border-slate-600 text-slate-100' : 'bg-editorial-cream border-editorial-charcoal text-editorial-charcoal'} border-2 p-4 absolute editorial-front flex flex-col justify-between`}>
          <div>
          ...
          <p className="text-xs font-mono self-end opacity-80">REVEAL MORE →</p>
        </div>
        
        {/* Editorial Back Face */}
        <div className={`w-full h-full border-2 border-slate-600 ${darkMode ? 'bg-slate-700 text-slate-100' : 'bg-editorial-cream text-editorial-charcoal'} p-4 absolute overflow-y-auto custom-scrollbar editorial-back flex flex-col`}>
          <button 
            onClick={handleRotate}
          ...
          <div className="flex-grow space-y-4 text-sm">
            ...
            {fallacyDefinition && (
              <div className="relative pl-4">
                ...
                <h6 className={`font-mono text-xs uppercase tracking-wider font-bold text-black ${darkMode ? 'text-slate-400' : 'text-editorial-charcoal opacity-70'} mb-1`}>Definition:</h6>
            ...
            <div className="relative pl-4">
              <div className={`absolute left-0 top-0 h-full w-0.5 ${darkMode ? 'bg-slate-600' : 'border-editorial-orange border-opacity-20 border-2'}`} />
              <h6 className={`font-mono text-xs uppercase tracking-wider font-bold text-black ${darkMode ? 'text-slate-400' : 'text-editorial-charcoal opacity-70'} mb-1`}>Analysis of Headline:</h6>
          ...
          <div className="mt-auto pt-4">
            <a href={currentArticle.url} target="_blank" rel="noopener noreferrer" className={`block w-full text-center ${darkMode ? 'bg-orange-500 text-slate-900 hover:bg-orange-400' : 'bg-editorial-cream border-2 border-editorial-orange text-editorial-orange hover:bg-editorial-orange text-editorial-cream transition-opacity'} py-2 font-mono text-xs font-bold`}>
